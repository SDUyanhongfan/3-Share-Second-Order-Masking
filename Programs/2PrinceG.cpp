...
f2 = cd ^ b
f3 = bd ^ cd ^ a ^ b
// the basic component boolean expression designed by the designer
f0_0=c1     || f1_0=c1^d1           || f2_0=d1&c1^b1       || f3_0=d1&c1^d1&b1^b1^a1  
f0_1=0      || f1_1=0               || f2_1=d1&c2          || f3_1=d1&c2^d1&b2     
f0_2=0      || f1_2=0               || f2_2=d1&c3          || f3_2=d1&c3^d1&b3          
f0_3=0      || f1_3=0               || f2_3=d2&c1          || f3_3=d2&c1^d2&b1             
f0_4=c2     || f1_4=c2^d2           || f2_4=d2&c2^b2       || f3_4=d2&c2^d2&b2^b2^a2       
f0_5=0      || f1_5=0               || f2_5=d2&c3          || f3_5=d2&c3^d2&b3                
f2_6=0      || f1_6=0               || f2_6=d3&c1          || f3_6=d3&c1^d3&b1                
f0_7=0      || f1_7=0               || f2_7=d3&c2          || f3_7=d3&c2^d3&b2             
f0_8=c3     || f1_8=c3^d3           || f2_8=d3&c3^b3       || f3_8=d3&c3^d3&b3^b3^a3  */
...
#define  EType_dANDbcXORab        2   //  this suits for the case of  d&b^a^c
...
{{"f3[0]=d1&c1^d1&b1^b1^a1 "},
 {"f3[1]=d1&c2^d1&b2     "},
 {"f3[2]=d1&c3^d1&b3       "},	
 {"f3[3]=d2&c1^d2&b1       "},	
 {"f3[4]=d2&c2^d2&b2^b2^a2 "},	
 {"f3[5]=d2&c3^d2&b3       "},
 {"f3[6]=d3&c1^d3&b1       "},	
 {"f3[7]=d3&c2^d3&b2       "},	
 {"f3[8]=d3&c3^d3&b3^b3^a3 "}	
};
...
/*
TransLinearItem_d3Sc123b123:   this   table  siuts  for  d&c ^b   or  d&b^c  
...
void InputTableIndexGen_NLdbLa(unsigned char  Type[4][9],  unsigned char CoordIndex, unsigned short  InitTranComLinearItem[4][9], unsigned char*   InputTableIndex[4][9],unsigned short  InitLinearItem[4][9],unsigned short  TransLinearItem[4][9][4])
{	
	unsigned int  CompIndex, InputMask,OutShare,i;
	unsigned char  abcd,a1b1c1d1,a2b2c2d2,a3b3c3d3;
	unsigned short	Masked_InputIndex,TypeIndex;
	   ...
	   for(a1b1c1d1=0;a1b1c1d1<16;a1b1c1d1++)
	    {
	    	...
	    	for(a2b2c2d2=0;a2b2c2d2<16;a2b2c2d2++)
	    	{
				...
				if ((InitTranComLinearItem[CoordIndex][1] &0xe00)==0x800)
				     InputTableIndex[CoordIndex][1][Masked_InputIndex]=(a1<<3)|(b2<<2)|(d1<<0);
				     ...
				     InputTableIndex[CoordIndex][1][Masked_InputIndex]=(a3<<3)|(b2<<2)|(d1<<0); 
                else
				...
				if ((InitTranComLinearItem[CoordIndex][3] &0xe00)==0x800)
				     ...
				     InputTableIndex[CoordIndex][3][Masked_InputIndex]=(a2<<3)|(b1<<2)|(d2<<0); 
				else if ((InitTranComLinearItem[CoordIndex][3] &0xe00)==0x200)
				     InputTableIndex[CoordIndex][3][Masked_InputIndex]=(a3<<3)|(b1<<2)|(d2<<0);  
				else
				...
				if ((InitTranComLinearItem[CoordIndex][8] &0xe00)==0x800)
				     InputTableIndex[CoordIndex][8][Masked_InputIndex]=(a1<<3)|(b3<<2)|(d3<<0);
				else if ((InitTranComLinearItem[CoordIndex][8] &0xe00)==0x400)
				     InputTableIndex[CoordIndex][8][Masked_InputIndex]=(a2<<3)|(b3<<2)|(d3<<0); 
	   ...
	   for(a1b1c1d1=0;a1b1c1d1<16;a1b1c1d1++)
	    {
	    	...
	    	for(a2b2c2d2=0;a2b2c2d2<16;a2b2c2d2++)
	    	{
				...
				// coordinate_1
				InputTableIndex[0][0][Masked_InputIndex]=(c1<<1);
				InputTableIndex[0][1][Masked_InputIndex]=0;
				InputTableIndex[0][2][Masked_InputIndex]=0;
				
				InputTableIndex[0][3][Masked_InputIndex]=0;
				InputTableIndex[0][4][Masked_InputIndex]=(c2<<1);
				InputTableIndex[0][5][Masked_InputIndex]=0;
				
				InputTableIndex[0][6][Masked_InputIndex]=0;
				InputTableIndex[0][7][Masked_InputIndex]=0;
				InputTableIndex[0][8][Masked_InputIndex]=(c3<<1);

				// coordinate_2
				InputTableIndex[1][0][Masked_InputIndex]=(c1<<1)|(d1<<0);
				InputTableIndex[1][1][Masked_InputIndex]=0;
				InputTableIndex[1][2][Masked_InputIndex]=0;
				
				InputTableIndex[1][3][Masked_InputIndex]=0;
				InputTableIndex[1][4][Masked_InputIndex]=(c2<<1)|(d2<<0);
				InputTableIndex[1][5][Masked_InputIndex]=0;
				
				InputTableIndex[1][6][Masked_InputIndex]=0;
				InputTableIndex[1][7][Masked_InputIndex]=0;
				InputTableIndex[1][8][Masked_InputIndex]=(c3<<1)|(d3<<0);		   
				...
				f2_1=d1&c1^b1   ^ XorValue[2][0][Type[2][0]][Masked_InputIndex];  
                f2_2=d1&c2        ^ XorValue[2][1][Type[2][1]][Masked_InputIndex] ; 
				f2_3=d1&c3        ^ XorValue[2][2][Type[2][2]][Masked_InputIndex] ; 

				F3FullMaskValue[2][0][Masked_InputIndex]=(f2_3<<2)|(f2_2<<1)|(f2_1<<0);
				F3To1UnMaskValue[2][0][Masked_InputIndex]=f2_1^f2_2^f2_3;

				f2_4= d2&c1          ^ XorValue[2][3][Type[2][3]][Masked_InputIndex]            ;
                f2_5= d2&c2^b2       ^ XorValue[2][4][Type[2][4]][Masked_InputIndex]            ;
				f2_6= d2&c3          ^ XorValue[2][5][Type[2][5]][Masked_InputIndex]            ;
				 
				F3FullMaskValue[2][1][Masked_InputIndex]=(f2_6<<2)|(f2_5<<1)|(f2_4<<0);
				F3To1UnMaskValue[2][1][Masked_InputIndex]=f2_6^f2_5^f2_4;

				f2_7=d3&c1         ^ XorValue[2][6][Type[2][6]][Masked_InputIndex]   ;
                f2_8=d3&c2         ^ XorValue[2][7][Type[2][7]][Masked_InputIndex]   ;
				f2_9=d3&c3^b3      ^ XorValue[2][8][Type[2][8]][Masked_InputIndex]    ;
				...
				f3_1=d1&c1^d1&b1^b1^a1      ^ XorValue[3][0][Type[3][0]][Masked_InputIndex]   ;
				f3_2=d1&c2^d1&b2          ^ XorValue[3][1][Type[3][1]][Masked_InputIndex]   ;
				f3_3=d1&c3^d1&b3            ^ XorValue[3][2][Type[3][2]][Masked_InputIndex]   ;
				
				F3FullMaskValue[3][0][Masked_InputIndex]=(f3_3<<2)|(f3_2<<1)|(f3_1<<0);
				F3To1UnMaskValue[3][0][Masked_InputIndex]=f3_1^f3_2^f3_3;

                f3_4= d2&c1^d2&b1           ^ XorValue[3][3][Type[3][3]][Masked_InputIndex] ;
                f3_5= d2&c2^d2&b2^b2^a2      ^ XorValue[3][4][Type[3][4]][Masked_InputIndex] ;
                f3_6= d2&c3^d2&b3            ^ XorValue[3][5][Type[3][5]][Masked_InputIndex] ;

				F3FullMaskValue[3][1][Masked_InputIndex]=(f3_6<<2)|(f3_5<<1)|(f3_4<<0);
				F3To1UnMaskValue[3][1][Masked_InputIndex]=f3_6^f3_5^f3_4;

                f3_7=d3&c1^d3&b1          ^ XorValue[3][6][Type[3][6]][Masked_InputIndex]  ;
                f3_8=d3&c2^d3&b2          ^ XorValue[3][7][Type[3][7]][Masked_InputIndex]  ;
                f3_9=d3&c3^d3&b3^b3^a3    ^ XorValue[3][8][Type[3][8]][Masked_InputIndex]  ;		
...
ExpressionType[2]= EType_dANDcXORb; 
