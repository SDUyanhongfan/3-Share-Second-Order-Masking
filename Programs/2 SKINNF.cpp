...
f0_0=c1&d1 ^ a1           || f1_0=c1&b1  ^ b1         || f2_0=b1+1    || f3_0=c1+1   
f0_1=c1&d2 ^ d2           || f1_1=c1&b2 ^d2             || f2_1=0       || f3_1=0    
f0_2=c1&d3 ^ c1           || f1_2=c1&b3 ^ c1            || f2_2=0       || f3_2=0       
f0_3=c2&d1 ^ c2           || f1_3=c2&b1 ^ c2            || f2_3=0       || f3_3=0   
f0_4=c2&d2 ^ a2           || f1_4=c2&b2 ^ b2            || f2_4=b2      || f3_4=c2    
f0_5=c2&d3 ^ d3           || f1_5=c2&b3 ^d3             || f2_5=0       || f3_5=0       
f2_6=c3&d1 ^ d1           || f1_6=c3&b1  ^ d1           || f2_6=0       || f3_6=0      
f0_7=c3&d2 ^ c3           || f1_7=c3&b2  ^ c3           || f2_7=0       || f3_7=0    
f0_8=c3&d3 ^ a3           || f1_8=c3&b3 ^ b3            || f2_8=b3      || f3_8=c3    */
...
//TableType
#define C3SameIntuple_a123d123    1  // this suits for  the case of d&c^a
#define C3SameIntuple_b123d123    2  // this suits for the case of  c&b^d
#define NoNeedTable    0

#define  C3SameIntuple_a123d123_Index 3

#define  C3SameIntuple_b123d123_Index 3

#define  NoNeedTable_Index 0

//ExpressionType
#define  EType_cANDdXORa         1  //  this suits for  the case of d&c^b or  d&c ^b^d^c  
#define  EType_cANDbXORd        2   //  this suits for the case of  d&b^a^c
...
comp_8     0         a3           d3    
----------------------------------------
TransLinearItem[][][]=(a1<<8)| (a2<<7)|(a3<<6)|(b1<<8)| (b2<<7)|(b3<<6)|(c1<<5)|(c2<<4)|(c3<<3)|(d1<<2)|(d2<<1)|(d3<<0); e.g. if b1=1：TransLinearItem[][][]=0x100, then this means that  the  item of "b1"  will used in the transformation  
*/

void TransLinearItem_c3Sa123d123(unsigned char CoordIndex,   unsigned short  TransLinearItem[4][9][4],unsigned char*   XorValue[4][9][4]) 
 {
...
comp_8     0          b3        d3         
-----------------------------------------
TransLinearItem[][][]=(a1<<8)| (a2<<7)|(a3<<6)|(b1<<8)|   (b2<<7)|(b3<<6)|(c1<<5)|(c2<<4)|          (c3<<3)|(d1<<2)|(d2<<1)|(d3<<0);
*/
void TransLinearItem_c3Sb123d123(unsigned char CoordIndex,   unsigned short  TransLinearItem[4][9][4],unsigned char*   XorValue[4][9][4]) 
 {
	 unsigned short	Masked_InputIndex,i;
	unsigned char  abcd,a1b1c1d1,a2b2c2d2,a3b3c3d3;
                ...
                for(CompIndex=0;CompIndex<9;CompIndex++)
	               {
	            	...
	            	if((CompIndex==0)||(CompIndex==3)||(CompIndex==6))
	            		  { 
						   XorValue[CoordIndex][CompIndex][1][Masked_InputIndex]=b1  ; //a1   type[1]   for comp0  comp3   comp6
	            	       XorValue[CoordIndex][CompIndex][2][Masked_InputIndex]= d1  ; //b1  type[2]   for comp0  comp3   comp6
	            		  } 
	            	else if((CompIndex==1)||(CompIndex==4)||(CompIndex==7))
	            		  { 
						   XorValue[CoordIndex][CompIndex][1][Masked_InputIndex]=b2  ; // a2  type[1]   for comp1  comp4   comp7
	            	       XorValue[CoordIndex][CompIndex][2][Masked_InputIndex]=d2    ; //b2  type[2]   for comp1  comp4   comp7
	            	       } 
	            	else 
	            		   {
						   XorValue[CoordIndex][CompIndex][1][Masked_InputIndex]=b3;//a3  type[1]   for comp2  comp5   comp8
	            	       XorValue[CoordIndex][CompIndex][2][Masked_InputIndex]=d3;//b3  type[2]   for comp2  comp5   comp8
	            	       } 
...
)// InitLinearItem[CompIndex]:CompIndex:0-8;  TransLinearItem[CompIndex][Type]:Type:0-7, XorValue[CompIndex][Type][Masked_InputIndex]: Masked_InputIndex:0-4095.
{
   ...
   for( CoordIndex=0;CoordIndex<4;CoordIndex++)
		{
			if(TableType[CoordIndex]==C3SameIntuple_a123d123)
			   TransLinearItem_c3Sa123d123(CoordIndex,   TransLinearItem, XorValue);
			else if(TableType[CoordIndex]==C3SameIntuple_b123d123)
			    TransLinearItem_c3Sb123d123(CoordIndex,   TransLinearItem, XorValue);
			else
...
void InputTableIndexGen_NLcdLa( unsigned char  Type[4][9], unsigned char CoordIndex, unsigned short  InitTranComLinearItem[4][9], unsigned char*   InputTableIndex[4][9],unsigned short  InitLinearItem[4][9],unsigned short  TransLinearItem[4][9][4])
{	
	   ...
	   for(a1b1c1d1=0;a1b1c1d1<16;a1b1c1d1++)
	    {
	    	...
	    	for(a2b2c2d2=0;a2b2c2d2<16;a2b2c2d2++)
	    	{
                ...
                if ((InitTranComLinearItem[CoordIndex][0] &0xe00)==0x800)
				     InputTableIndex[CoordIndex][0][Masked_InputIndex]=(a1<<2)|(c1<<1)|(d1<<0);
				...
				//---component 1
				 if ((InitTranComLinearItem[CoordIndex][1] &0xe00)==0x400)
				     InputTableIndex[CoordIndex][1][Masked_InputIndex]=(a2<<2)|(c1<<1)|(d2<<0); 
				
                else
                    InputTableIndex[CoordIndex][1][Masked_InputIndex]=(c1<<1)|(d2<<0); 	
					//printf("InputTableIndex[%d][1][%d]:%d \n",CoordIndex,Masked_InputIndex,InputTableIndex[CoordIndex][0][Masked_InputIndex]);				
				//---component 2 
				if ((InitTranComLinearItem[CoordIndex][2] &0xe00)==0x200)
				     InputTableIndex[CoordIndex][2][Masked_InputIndex]=(a3<<2)|(c1<<1)|(d3<<0);  
				else
					 InputTableIndex[CoordIndex][2][Masked_InputIndex]=(c1<<1)|(d3<<0);  
				 
                //---component 3
				if ((InitTranComLinearItem[CoordIndex][3] &0xe00)==0x800)
				     InputTableIndex[CoordIndex][3][Masked_InputIndex]=(a1<<2)|(c2<<1)|(d1<<0);
				
				else
					InputTableIndex[CoordIndex][3][Masked_InputIndex]=(c2<<1)|(d1<<0); 
				 
				 //---component 4

				 if ((InitTranComLinearItem[CoordIndex][4] &0xe00)==0x400)
				     InputTableIndex[CoordIndex][4][Masked_InputIndex]=(a2<<2)|(c2<<1)|(d2<<0); 
				
				 else
					InputTableIndex[CoordIndex][4][Masked_InputIndex]=(c2<<1)|(d2<<0); 
				//---component 5
				if ((InitTranComLinearItem[CoordIndex][5] &0xe00)==0x200)
				     InputTableIndex[CoordIndex][5][Masked_InputIndex]=(a3<<2)|(c2<<1)|(d3<<0);
				else 
					InputTableIndex[CoordIndex][5][Masked_InputIndex]=(c2<<1)|(d3<<0);

                //---component 6
				if ((InitTranComLinearItem[CoordIndex][6] &0xe00)==0x800)
				     InputTableIndex[CoordIndex][6][Masked_InputIndex]=(a1<<2)|(c3<<1)|(d1<<0);
				
				else 
          			InputTableIndex[CoordIndex][6][Masked_InputIndex]=(c3<<1)|(d1<<0);		
				 //---component 7
				 if ((InitTranComLinearItem[CoordIndex][7] &0xe00)==0x400)
				     InputTableIndex[CoordIndex][7][Masked_InputIndex]=(a2<<2)|(c3<<1)|(d2<<0); 

				else 
          			InputTableIndex[CoordIndex][7][Masked_InputIndex]=(c3<<1)|(d2<<0);
                //---component 8
				 if ((InitTranComLinearItem[CoordIndex][8] &0xe00)==0x200)
				     InputTableIndex[CoordIndex][8][Masked_InputIndex]=(a3<<2)|(c3<<1)|(d3<<0);
				else 
...
void InputTableIndexGen_NLcbLd(unsigned char  Type[4][9],  unsigned char CoordIndex, unsigned short  InitTranComLinearItem[4][9], unsigned char*   InputTableIndex[4][9],unsigned short  InitLinearItem[4][9],unsigned short  TransLinearItem[4][9][4])
{	
	unsigned int  CompIndex, InputMask,OutShare,i;
	unsigned char  abcd,a1b1c1d1,a2b2c2d2,a3b3c3d3;
	unsigned short	Masked_InputIndex,TypeIndex;
	   ...
	   for(a1b1c1d1=0;a1b1c1d1<16;a1b1c1d1++)
	    {
	    	...
	    	for(a2b2c2d2=0;a2b2c2d2<16;a2b2c2d2++)
	    	{
                ...
                if ((InitTranComLinearItem[CoordIndex][0] &0x003)==0x004)
					 ...
					 InputTableIndex[CoordIndex][0][Masked_InputIndex]=(b1<<2)|(c1<<1); 
				//---component 1  
				if ((InitTranComLinearItem[CoordIndex][1] &0x003)==0x002)
				     InputTableIndex[CoordIndex][1][Masked_InputIndex]=(b2<<2)|(c1<<1)|(d2<<0); 
                else
                    InputTableIndex[CoordIndex][1][Masked_InputIndex]=(b2<<2)|(c1<<1); 				
				//---component 2   
				 if ((InitTranComLinearItem[CoordIndex][2] &0x003)==0x001)
				     InputTableIndex[CoordIndex][2][Masked_InputIndex]=(b3<<2)|(c1<<1)|(d3<<0);  
				else
					 InputTableIndex[CoordIndex][2][Masked_InputIndex]=(b3<<2)|(c1<<1);  
				 
                //---component 3 
				if ((InitTranComLinearItem[CoordIndex][3] &0x003)==0x004)
				     InputTableIndex[CoordIndex][3][Masked_InputIndex]=(b1<<2)|(c2<<1)|(d1<<0);
			 
				else
					InputTableIndex[CoordIndex][3][Masked_InputIndex]=(b1<<2)|(c2<<1);
				 
				 //---component 4 

				 if ((InitTranComLinearItem[CoordIndex][4] &0x003)==0x002)
				     InputTableIndex[CoordIndex][4][Masked_InputIndex]=(b2<<2)|(c2<<1)|(d2<<0); 
				
				 else
					InputTableIndex[CoordIndex][4][Masked_InputIndex]=(b2<<2)|(c2<<1);
				//---component 5 
				 if ((InitTranComLinearItem[CoordIndex][5] &0x003)==0x001)
				     InputTableIndex[CoordIndex][5][Masked_InputIndex]=(b3<<2)|(c2<<1)|(d3<<0);
				else 
					InputTableIndex[CoordIndex][5][Masked_InputIndex]=(b3<<2)|(c2<<1);

                //---component 6 
				if ((InitTranComLinearItem[CoordIndex][6] &0x003)==0x004)
				     InputTableIndex[CoordIndex][6][Masked_InputIndex]=(b1<<2)|(c3<<1)|(d1<<0);
				else 
          			InputTableIndex[CoordIndex][6][Masked_InputIndex]=(b1<<2)|(c3<<1);	
				 //---component 7 
				if ((InitTranComLinearItem[CoordIndex][7] &0x003)==0x002)
				     InputTableIndex[CoordIndex][7][Masked_InputIndex]=(b2<<2)|(c3<<1)|(d2<<0); 
				
				else 
          			InputTableIndex[CoordIndex][7][Masked_InputIndex]=(b2<<2)|(c3<<1);
                //---component 8  
				if ((InitTranComLinearItem[CoordIndex][8] &0x003)==0x001)
				     InputTableIndex[CoordIndex][8][Masked_InputIndex]=(b3<<2)|(c3<<2)|(d3<<0);
				else 
          			InputTableIndex[CoordIndex][8][Masked_InputIndex]=(b3<<2)|(c3<<1);
...
void MakeLargeTables( unsigned char  Type[4][9], unsigned char*     InputTableIndex[4][9], unsigned char*     F3FullMaskValue[4][3],unsigned char*     F3To1UnMaskValue[4][3],unsigned char*   XorValue[4][9][4],unsigned short  InitLinearItem[4][9],unsigned short  TransLinearItem[4][9][4],unsigned short  InitTranComLinearItem[4][9],unsigned char ExpressionType[4])
{
 ...
 for(CoordIndex=0;CoordIndex<4;CoordIndex++)
	 {
		if(ExpressionType[CoordIndex]==EType_cANDdXORa)
			InputTableIndexGen_NLcdLa(Type,CoordIndex,InitTranComLinearItem, InputTableIndex,InitLinearItem,TransLinearItem);
		else if(ExpressionType[CoordIndex]==EType_cANDbXORd)
			InputTableIndexGen_NLcbLd(Type,CoordIndex,InitTranComLinearItem, InputTableIndex,InitLinearItem,TransLinearItem);
	 }
	   ...
	   for(a1b1c1d1=0;a1b1c1d1<16;a1b1c1d1++)
	    {
	    	...
	    	for(a2b2c2d2=0;a2b2c2d2<16;a2b2c2d2++)
	    	{
				...
				// coordinate_1
				InputTableIndex[2][0][Masked_InputIndex]=(b1<<0);
				InputTableIndex[2][1][Masked_InputIndex]=0;
				InputTableIndex[2][2][Masked_InputIndex]=0;
								
				InputTableIndex[2][3][Masked_InputIndex]=0;
				InputTableIndex[2][4][Masked_InputIndex]=(b2<<0);
				InputTableIndex[2][5][Masked_InputIndex]=0;
								
				InputTableIndex[2][6][Masked_InputIndex]=0;
				InputTableIndex[2][7][Masked_InputIndex]=0;
				InputTableIndex[2][8][Masked_InputIndex]=(b3<<0);
				
				InputTableIndex[3][0][Masked_InputIndex]=(c1<<0);
				InputTableIndex[3][1][Masked_InputIndex]=0;
				InputTableIndex[3][2][Masked_InputIndex]=0;
								
				InputTableIndex[3][3][Masked_InputIndex]=0;
				InputTableIndex[3][4][Masked_InputIndex]=(c2<<0);
				InputTableIndex[3][5][Masked_InputIndex]=0;
								
				InputTableIndex[3][6][Masked_InputIndex]=0;
				InputTableIndex[3][7][Masked_InputIndex]=0;
				InputTableIndex[3][8][Masked_InputIndex]=(c3<<0);
				/* 2 need to  change the   after fi_j =   the expressions */


				
                 // coordinate_0
				f0_1=c1&d1   ^ a1 ^1   ^ XorValue[0][0][Type[0][0]][Masked_InputIndex];  
                f0_2=c1&d2 ^d2         ^ XorValue[0][1][Type[0][1]][Masked_InputIndex] ; 
                f0_3=c1&d3 ^ c1        ^ XorValue[0][2][Type[0][2]][Masked_InputIndex] ; 

				F3FullMaskValue[0][0][Masked_InputIndex]=(f0_3<<2)|(f0_2<<1)|(f0_1<<0);
				F3To1UnMaskValue[0][0][Masked_InputIndex]=f0_1^f0_2^f0_3;

				f0_4=c2&d1 ^ c2      ^ XorValue[0][3][Type[0][3]][Masked_InputIndex];
                f0_5=c2&d2 ^ a2      ^ XorValue[0][4][Type[0][4]][Masked_InputIndex];
                f0_6=c2&d3 ^d3       ^ XorValue[0][5][Type[0][5]][Masked_InputIndex];
                

				F3FullMaskValue[0][1][Masked_InputIndex]=(f0_6<<2)|(f0_5<<1)|(f0_4<<0);
				F3To1UnMaskValue[0][1][Masked_InputIndex]=f0_6^f0_5^f0_4;
				
				f0_7= c3&d1 ^ d1    ^ XorValue[0][6][Type[0][6]][Masked_InputIndex];
                f0_8= c3&d2 ^ c3    ^ XorValue[0][7][Type[0][7]][Masked_InputIndex];
                f0_9= c3&d3 ^ a3    ^ XorValue[0][8][Type[0][8]][Masked_InputIndex];
          
				F3FullMaskValue[0][2][Masked_InputIndex]=(f0_9<<2)|(f0_8<<1)|(f0_7<<0);
				F3To1UnMaskValue[0][2][Masked_InputIndex]=f0_9^f0_8^f0_7;

				// coordinate_1
				f1_1=c1&b1   ^ b1 ^ XorValue[1][0][Type[1][0]][Masked_InputIndex]^1;  
                f1_2=c1&b2 ^d2    ^ XorValue[1][1][Type[1][1]][Masked_InputIndex] ; 
                f1_3=c1&b3 ^ c1   ^ XorValue[1][2][Type[1][2]][Masked_InputIndex] ; 

				F3FullMaskValue[1][0][Masked_InputIndex]=(f1_3<<2)|(f1_2<<1)|(f1_1<<0);
				F3To1UnMaskValue[1][0][Masked_InputIndex]=f1_1^f1_2^f1_3;

				f1_4=c2&b1 ^ c2   ^ XorValue[1][3][Type[1][3]][Masked_InputIndex];
                f1_5=c2&b2 ^ b2   ^ XorValue[1][4][Type[1][4]][Masked_InputIndex];
                f1_6=c2&b3 ^d3    ^ XorValue[1][5][Type[1][5]][Masked_InputIndex];
                

				F3FullMaskValue[1][1][Masked_InputIndex]=(f1_6<<2)|(f1_5<<1)|(f1_4<<0);
				F3To1UnMaskValue[1][1][Masked_InputIndex]=f1_6^f1_5^f1_4;
				
				f1_7= c3&b1 ^ d1  ^ XorValue[1][6][Type[1][6]][Masked_InputIndex];
                f1_8= c3&b2 ^ c3  ^ XorValue[1][7][Type[1][7]][Masked_InputIndex];
                f1_9= c3&b3 ^ b3  ^ XorValue[1][8][Type[1][8]][Masked_InputIndex];
          
				F3FullMaskValue[1][2][Masked_InputIndex]=(f1_9<<2)|(f1_8<<1)|(f1_7<<0);
				F3To1UnMaskValue[1][2][Masked_InputIndex]=f1_9^f1_8^f1_7;
				
				
			    // coordinate_2 initial expression+ possible linear item in the search process
				...
				// coordinate_3 initial expression+ possible linear item in the search process
				f3_1= c1^1 ;         
				f3_2= 0     ;
				f3_3= 0   ;
...
TableType[0]=C3SameIntuple_a123d123;
TableType[1]=C3SameIntuple_b123d123;
TableType[2]=NoNeedTable;
TableType[3]=NoNeedTable;


TableTypeIndex[0]= C3SameIntuple_a123d123_Index;
TableTypeIndex[1]= C3SameIntuple_b123d123_Index;
...
ExpressionType[0]= EType_cANDdXORa;
ExpressionType[1]= EType_cANDbXORd; 
				...
				for(int k=0;k<CaseNum[CoordIndex3];k++)
					{
						...
						fprintf(F, "========================================\n"); 
						   for (i=0;i<9;i++)
						   {
							fprintf(F, "%s %s; \n",f0BaseExpression[i],f0AddItems[i][Type[CoordIndex1][i]]);
						   }
						  fprintf(F, "========================================\n");
